...
**Docker** (`Dockerfile` + `docker-compose.yml`): `docker compose up -d` runs **two** services. `graphhopper` builds the multi-stage image (Maven → `eclipse-temurin:25-jre`, ~577MB) on 8989/8990, bind-mounting `./graph-cache` (persists, no re-import) and the PBF read-only; its healthcheck is `bash` + `/dev/tcp` because the JRE image has **no curl or wget**, with `start_period: 300s` for the cold graph load. `wrapper` builds `incident-wrapper/Dockerfile` (`python:3.12-slim`) on 8000, reaches the engine as `http://graphhopper:8989` (service name, not localhost), takes `BAATO_KEY` at runtime from the gitignored `incident-wrapper/.env` (`env_file … required: false`, so the stack starts without a key) and keeps it out of the image via `incident-wrapper/.dockerignore`. The incident DB is a **directory** mount `./incident-wrapper/data:/data` + `INCIDENTS_DB=/data/incidents.db` — a single-file bind mount would be created as a directory on a fresh clone and SQLite needs its journal alongside the db; `run.sh` defaults to the same `data/incidents.db` so host and container runs share one database. **`static/` is baked into the wrapper image**, so a dashboard edit needs `docker compose build wrapper && docker compose up -d wrapper`, not just a browser refresh. The GraphHopper entrypoint republishes the connectors on 0.0.0.0 with **snake_case** Dropwizard overrides (`-Ddw.server.application_connectors[0].bind_host=0.0.0.0`) — camelCase override keys fail with `node with index not found`.
